...
class NewsArticles {
    ...
    function validate() {
		...
		if (false === strtotime($this->articleData['articleDate'])){
    ...
    function getList() {
		...
		$stmt = $this->db->query("SELECT articleID, articleTitle, articleContent, articleAuthor, articleDate FROM newsarticles");
		
		while($articleList = $stmt->fetch(PDO::FETCH_ASSOC)){
			
			echo($articleList['articleTitle']);
			echo($articleList['articleContent']);
			echo($articleList['articleAuthor']);
			echo($articleList['articleDate']);
			echo("<a href=../public/article-view.php?articleID=$articleList[articleID]><button>View</button> </a>");
			echo("<a href=../public/article-edit.php?articleID=$articleList[articleID]><button>Edit</button> </a>");
			echo("</br>");
